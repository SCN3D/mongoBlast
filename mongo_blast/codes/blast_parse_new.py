...
def get_ptms(ptm,table,s_p,e_p,insertions,deletions,seqs):
    ...
    for id in seqs:
        ...
        if ptm in data:
            for i in data[ptm]:
                if id == 'KKCC2_HUMAN':
                    print("id is "+id+"  ptm is "+str(i))
                    print("start: "+ str(s_p[id]) + " end:" + str(e_p[id]))
                if int(i) >= s_p[id] and int(i) <= e_p[id]:
                    
                    out_ptm = int(i) - s_p[id] + pad + 1
                    if id == 'KKCC2_HUMAN':
                        print("id: "+str(id)+"   outptm before insert: "+str(out_ptm))
                            ...
                            if j[0] < k.pos:
                                ...
                                if out_ptm > k.pos: 
                                    if out_ptm <= k.pos+len(k.seq):
                                        if id == 'KKCC2_HUMAN':
                                            print("outptm now:"+str(out_ptm)+" delet start postion: "+str(k.pos))
                                        delete = True
                                        break
                                    else:
                                        out_ptm -= len(k.seq)
                    ...
                    # if only have delete
                        ...
                        if id == 'KKCC2_HUMAN':
                            print("outptm now:"+str(out_ptm)+" calc deletion...")
